 ...
 */
public class CheckInAppointmentHandler extends BaseCommand {

	private static final Logger LOGGER = LogManager.getLogger(CheckInAppointmentHandler.class);

	@Override
	public ApiMessage execute() {
		MonitorStats.createThreadMonitorStats("CheckInAppointmentHandler");
		try {
			...
			MsgAppointmentCheckIn msg = MsgAppointmentCheckIn.convertObject(data);

			ApiMessage apiMessage;
			if (msg == null) {
				apiMessage = ApiMessage.INVALID_DATA;
			} else {
				apiMessage = PlaceModel.instance.checkInAppointment(uid, msg);
