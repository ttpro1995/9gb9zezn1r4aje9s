 ...
 */
public class RemovePlaceHandler extends BaseCommand {

    private static final Logger LOGGER = LogManager.getLogger(RemovePlaceHandler.class);

    @Override
    public ApiMessage execute() {
        MonitorStats.createThreadMonitorStats("RemovePlaceHandler");
        try {
            ...
            MsgRemovePlace msg = MsgRemovePlace.convertObject(data);

            ApiMessage apiMessage;
            if (msg == null) {
                apiMessage = ApiMessage.INVALID_DATA;
            } else {
                apiMessage = PlaceModel.instance.removePlace(uid, msg);
