...
public class FriendRequestingListFragment extends Fragment implements AdapterView.OnItemClickListener, AbsListView.OnScrollListener {
    ...
    public void onItemClick(AdapterView<?> parent, View view, final int position, long id) {
        ViewUtils.createConfirmDialog(getContext(), "Xác nhận là bạn bè?", "Chấp nhận", "Từ chối", new DialogInterface.OnClickListener() {
            @Override
    ...
    private void nextPage() {
        ...
        loading = true;
        RestAPI.getListRequesting(DataUtils.getToken(), new RestAPI.Callback() {
            @Override
    ...
    public void acceptFriend(final int position) {
        String json = new JSONStringBuilder()
                ...
                .create();
        RestAPI.acceptFriend(json, DataUtils.getToken(), new RestAPI.Callback() {
            @Override
            public void onDone(JSONObject responseData, boolean hasError) {
                if (hasError) {
    ...
    public void denyFriend(final int position) {
        String json = new JSONStringBuilder()
                .put("uidTo", ((Friend) adapter.getItem(position)).id)
                .create();
        RestAPI.denyFriend(json, DataUtils.getToken(), new RestAPI.Callback() {
            @Override
            public void onDone(JSONObject responseData, boolean hasError) {
                if (hasError) {
                    Toast.makeText(getContext(), "Đã xảy ra lỗi.", Toast.LENGTH_SHORT).show();
